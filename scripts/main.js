...
$(window).load(function(){
  ...
  if ($('#cycle-detail-info').length)
  {
    ...
    var preloaderCount = 1;

    $('#cycle-detail-img').after('<div id="preload" style="height: 0px; width: 100%; overflow: hidden; opacity: 0;"></div>');
    $('#preload').html(detailSlides[0]);
    $('#cycle-detail-img').on('cycle-next', function(event, hash){
      var slideNum = hash.currSlide;
      if(slideNum >= detailSlides.length)
      {
        slideNum = 0;
      }
      $('#preload').html(detailSlides[slideNum]);
    });
  ...
  if ($('#cycle-index-img').length)
  {
    ...
    $.each(_indexSlides, function(index, val){
      ...
      imgSlide += '<a class="item-link" href="'+menuLink+'">Zobraziť viac</a></div>';
      imgSlide += '</div>';
